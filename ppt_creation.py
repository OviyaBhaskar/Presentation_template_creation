...
def call_groq(prompt):
    url = "/llama38b"  # Add actual API URL here
    payload = json.dumps({
        "prompt": prompt,
        "max_tokens": 2048,
        "temperature": 0.1,
...
def create_presentation(template_path, output_path, main_title, subtitle, slides_content, two_column_slides):
    ...
    max_words_per_slide = 70  # Adjust this number based on template's capacity
...
def main():
    file_path = r'D:\llm_group_app\Presentation_templates\templates_groq\Knowledge_Graphs_15pages.pdf'
    ...
    templates_directory = r"D:\llm_group_app\Presentation_templates\templates_groq"

    templates = load_templates(templates_directory)
    selected_template_path = select_template(templates)
    output_pptx_path = r"D:\llm_group_app\Presentation_templates\templates_groq\output_presentation.pptx"
